...
Dado("que o usuário informou todos os dados da entrada e o tipo da nota é {string}") do |tipo_nota|
  EntradaManual_dados_iniciais.new.preencherDadosIniciais_tiposNota(tipo_nota, 1010, 1)

  # Salvar nos dados iniciais
  sleep 2
  @@botoes_entrada.clicarBotao
end
...
Dado("que a conciliação de um dos itens do XML é realizada para dois itens do sistema:") do |table|
  ...
  XMLConciliacao.new.conciliarItens(@produto_conciliacao)
end
...
Quando('finaliza a entrada') do
  @@botoes_entrada.prosseguirAteFinalizar
end
...
Quando("o usuário informar na entrada o tipo de nota {string} com número do documento {string} e série {string} informando chave de acesso") do |tipo_nota, doc, serie|
  EntradaManual_dados_iniciais.new.preencherDadosIniciais_tiposNota(tipo_nota, doc, serie)

  chaveNFe = find('input[formcontrolname=chaveNfe]')
  chaveNFe.set '42181275552133001303550040000005621722790267'
  @@campos_entrada.setarDataEmissao('20122018')
