...
class DB{    
    ...
    consulta_escrituracao(){
       const query = `select * from (
                    select nf.ESCRITURADA from QA_ESTOQUE_AUTO.entrada entrada
                    inner join QA_ESTOQUE_AUTO.entrada_nota_fiscal entrada_nf
                    on entrada.ID = entrada_nf.ENTRADA_ID
                    inner join QA_ESTOQUE_AUTO.NOTA_FISCAL nf
    ...
    consulta_ultimaEntrada(){
        const query = `select * from (QA_ESTOQUE_AUT.entrada
                    order by nf.id desc)
            where ROWNUM <= 1`
        return consulta_banco(query)
        }
