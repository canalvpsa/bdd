...
Cenário: Validar permissão de usuário para cadastrar balanço
    ...
    Então é exibida opção para criar
...
Cenário: Processar balanço na pesquisa do balanço
    ...
    E que possui um balanço não processado na listagem
    Quando acessa as opções do balanço de estoque
    Então pode processar o balanço
...
Cenário: Excluir balanço não processado
    Dado que o usuário selecionou balanço com status "pendente"
...
Cenário: Excluir balanço processado sem nota(s) fiscal(is) emitida(s)
    ...
    E os lançamentos contábeis excluídos
    E o balanço é excluído
